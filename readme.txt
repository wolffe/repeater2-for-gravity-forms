...
Contributors: butterflymedia, KodieGrantham
...
* Use Javascript to manipulate the repeater2 (See GitHub Page)
...
* Fixed bug where sometimes captured repeater2 input data would be applied to other forms in cases where there are multiple forms on one page.
...
* Field input masks are now stored in gfRepeaters data. (gfRepeaters[repeater2Id]['children'][childId]['inputMask'])
...
= 1.0.7.2 =
* Fixed saved repeater2 field data during validation. (Accidently broke it in the 1.0.7.1 patch)

= 1.0.7.1 =
* Fixed a bug where repeater2 would break multipage forms. (Sorry about that!)

= 1.0.7 =
* Fixed bug where field order was not saved when a form was submitted.
* Fixed syntax error for older versions of PHP.
* Repeated fields now have their tabindex set to the one that was assigned to the repeater2 start field.
* Repeater start field tabindex now stored in gfRepeaters data. (gfRepeaters[repeater2Id]['data']['tabIndex'])
* Repeater is now repeated if a paremeter is set for a specific field that is higher than the start repeat count.
* Highest set paremeter number is now stored in gfRepeaters data. (gfRepeaters[repeater2Id]['data']['paremCount'])
* Repeater dependencies, actions, and filters are now only loaded on forms that have a repeater2 field.
* Field type is now set for hidden and section fields in gfRepeaters data.
* Ajax is now automatically disabled on forms that have a repeater2 field. (Until I come up with a fix for ajax enabled forms)
* Added support for Andy Fragen's GitHub Updater. (See readme for more info)
* CSS is now minified.
* gfRepeaters data now contains the form ID number as a root and the repeater2 Id as a subroot. (See readme for more info)

= 1.0.6 =
* Fixed repeater2 'start' setting. (Apparently it's been broken for awhile?)
* Fixed datepicker ui for repeated date fields.
* Fixed bug where repeater2 plugin wouldn't allow page settings to display in the form editor.
...
* Added the ability to change the repeater2 'start' setting by using the built in Gravity Froms pre-populate filters and hooks.
...
* Field ID number is now stored in gfRepeaters data. (gfRepeaters[repeater2Id]['children'][childId]['idNum'])
* Field input pre-populate value is now stored in GfRepeaters data. (gfRepeaters[repeater2Id]['children'][childId]['inputs'][inputId]['prePopulate']) (See readme for more info)
...
* Field types are now stored in gfRepeaters data. (gfRepeaters[repeater2Id]['children'][childId]['type'])
...
* Fixed bug where a javascript error would occur if no repeater2 children were marked as required.
...
* Default validation functions are now bypassed for repeater2 children.
...
* Repeater children and repeater2 end fields no longer display duplicated results on the entries page in the admin panel.
* Added a little bit of backwards compatibility for fetching saved repeater2 child entry data.
...
* Fixed bugs with form editor and setting repeater2 children required option.
* Added beforeRepeat, afterRepeat, beforeUnRepeat, and AfterUnRepeat form triggers.
* Added a stylesheet with some default styles for repeater2 releated stuff.
* Added keypress event and tab index to add and remove buttons.
* Added a little bit of error handing to the javascript just in case the admin forgets to add a repeater2-end or trys to nest repeater2s.
* Repeater start field label now defaults to blank instead of "Untitled".

= 1.0.2 =
* Fixed bug where if any fields in a form with a repeater2 in it would be unsubmittable.
* Fixed repeater2 field validation.
* Added custom validation message feature.
* Added repeater2 "start" number feature.
* Added repeater2 label and description options.
